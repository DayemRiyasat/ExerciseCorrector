# exercises/pushup/RepCounter.py
...
class PushupRepCounter:
    """Advanced rep counter for push-ups"""
    
    def __init__(self, form_threshold=0.10):
        self.rep_count = 0
        self.state = "up"  # States: up, descending, bottom, ascending
        self.form_threshold = form_threshold
        self.form_scores = []
        
        # Elbow angle thresholds
        self.elbow_angle_bottom = 90   # Bent elbows at bottom
        self.elbow_angle_top = 160     # Extended arms at top
    ...
    def update(self, landmarks, prediction, confidence):
        """Update rep counter based on elbow angles"""
        try:
            # Get key landmarks
            ...
            if self.state == "up":
                ...
                if avg_elbow_angle <= self.elbow_angle_bottom:
                    self.frames_in_current_state += 1
                    if self.frames_in_current_state >= self.min_frames_in_state:
                        self.state = "bottom"
                        state_changed = True
                        self.frames_in_current_state = 0
                # If starting to go back up before reaching bottom
                elif avg_elbow_angle > self.lowest_angle + 10:
                    self.state = "up"
                    self.form_scores = []
                    self.frames_in_current_state = 0
                else:
                ...
                # Starting to push up
                if avg_elbow_angle > self.lowest_angle + 10:
                    self.frames_in_current_state += 1
                    if self.frames_in_current_state >= self.min_frames_in_state:
                ...
                if avg_elbow_angle >= self.elbow_angle_top:
                    self.frames_in_current_state += 1
                    if self.frames_in_current_state >= self.min_frames_in_state:
                        if current_time - self.last_rep_time >= self.min_rep_interval:
                            ...
                            # Count rep if form is good enough
                        ...
                        return {
                            ...
                            'depth_achieved': self.lowest_angle,
